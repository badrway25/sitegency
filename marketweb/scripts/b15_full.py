"""Batch 15: 7 families — full pipeline with light audit."""
...
FAMILIES = ['animal', 'app-landing', 'architects', 'automotive', 'barber', 'blog', 'charity']
...
with sync_playwright() as p:
    ...
    for fam in FAMILIES:
        ...
        for slug in slugs:
            ...
            time.sleep(6)
            page.screenshot(path=os.path.join(OUTDIR, f'{slug}_b15.png'), full_page=True)
            ...
            tpl['desk'] = page.evaluate("""() => {
                ...
                // Light audit: count weak sections
            ...
            if d['emptyBlocks'] > 1: issues.append(f"EMPTY:{d['emptyBlocks']}")
            s = ' '.join(issues) if issues else 'OK'
            print(f'  {slug}: nav={d["navH"]}px ftco={d["ftco"]} imgs={d["imgs"]} h2={d["h2"]} weak={d["weakSections"]}/{d["totalSections"]} mob={tpl["mob"]["overflow"]} ar={tpl["ar"]["dir"]} {s}')

    browser.close()

with open('docs/b15_audit.json', 'w') as f:
...
print(f'TOTAL: {total} templates | {clean}/{total} clean | {total_weak} weak sections / {total_sections} total ({round(total_weak/max(total_sections,1)*100,1)}%)')
...
for fam in FAMILIES:
    ...
    print(f'  {fam}: {c}/{t} clean | weak_avg={w/t:.1f} | ftco={ftco_v}/{ftco_t}')

# Micro-polish candidates (worst 30%)
print('\nMICRO-POLISH CANDIDATES:')
ranked = sorted(ALL.items(), key=lambda x: -x[1]['desk']['weakSections'])
target = max(1, int(total * 0.30))
for i, (slug, v) in enumerate(ranked[:target]):
    d = v['desk']
    if d['weakSections'] > 0:
        print(f'  {slug}: weak={d["weakSections"]}/{d["totalSections"]} h={d["h"]}')
